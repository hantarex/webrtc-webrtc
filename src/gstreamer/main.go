...
func (g *GStreamer) InitGst() {
	...
	g.pipeline = C.gst_parse_launch(C.CString("videotestsrc ! queue2 ! videoconvert ! video/x-raw,format=NV12 ! nvh264enc ! video/x-h264,profile=baseline ! queue2 ! rtph264pay pt=96 ! webrtcbin stun-server=stun://stun.l.google.com:19302 name=sendrecv"), &g.gError)
	// webrtcbin1
